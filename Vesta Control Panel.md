...
Mục đích là sử dụng Certbot (một công cụ độc lập của Let's Encrypt) để quản lý SSL thay vì phụ thuộc hoàn toàn vào bảng điều khiển (Control Panel). Điều này giúp chủ động trong việc gia hạn và bảo mật đường truyền (HTTPS) cho mọi website trên server.
...
Thay vì dùng các nút bấm "mì ăn liền" trên CyberPanel, việc chuyển sang VestaCP buộc phải can thiệp vào
...
Mặc dù VestaCP đã cài xong, nhưng bản PHP đi kèm thường là bản cũ. chạy các lệnh này để đưa nó lên 8.1
...
# Cấp quyền để upload
ảnh và cài plugin không bị hỏi mật khẩu FTP
...
Apache là thằng xử lý file PHP phía sau. cần sửa cả file chạy thường và file SSL.
...
Nginx đóng vai trò là "người gác cổng" phía trước. cũng cần sửa 2 file tương tự.
...
Chính vì VestaCP của mình  không có cái nút ép HTTPS tự động đó, nên cái "liều thuốc" tui kê cho đưa vào file wp-config.php là bắt buộc 100% phải có để trị dứt điểm cái bệnh "Vòng lặp 301".
...
🛠️ Bước 2: Sau khi code đã ra đúng chỗ, dán mống này vào:
...
chạy 2 lệnh này cho 2 tên miền tương ứng (Nhớ chạy với quyền root nhé):
